...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    const anyObj = { 'food':
      {'coxinha': 3.9, 'sopa': 9.9}, 'drinks': {'agua': 3.9, 'cerveja': 6.9 }}
    const expectedObj = { 
      fetchMenu: anyObj,
      order: () => {},
      consumption: []
    }
    const respReturn = createMenu(anyObj)
    assert.deepEqual(respReturn.fetchMenu, expectedObj.fetchMenu)
    ...
    assert.deepEqual(respReturn.consumption, [])
    // --------------------------------------------------------------------------------------
    ...
    respReturn.order('coxinha')
    assert.deepEqual(respReturn.consumption, ['coxinha'])
    // --------------------------------------------------------------------------------------
    ...
    // ```
    respReturn.consumption = []
    respReturn.order('coxinha')
    respReturn.order('agua')
    respReturn.order('sopa')
    respReturn.order('sashimi')
    assert.deepEqual(respReturn.consumption, ["coxinha", "agua", "sopa", "sashimi"])
    ...
    respReturn.order('agua')
    respReturn.order('coxinha')
    assert.deepEqual(respReturn.consumption, ["coxinha", "agua", "coxinha"])
   ...
   respReturnn.order('coxinha')
   console.log(respReturnn.consumption) 
   assert.deepEqual(respReturnn.pay(), total)
