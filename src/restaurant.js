...
const checkProductIn = (product, menuEntryObj) => {
  const objectKeys = Object.keys(menuEntryObj);
  return objectKeys.includes(product);
};
...
const createMenu = menu => ({
  ...
  pay() {
    ...
    for (let index = 0; index < this.consumption.length; index += 1) {
      const product = this.consumption[index];
