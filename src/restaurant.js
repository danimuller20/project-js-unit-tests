...
const find = (item, categories, category) => {
  let priceItem = 0;
...
const pay = () => {
  ...
  restaurant.consumption.forEach((item) => {
    price += find(item, Object.keys(restaurant.fetchMenu.food), 'food');
    price += find(item, Object.keys(restaurant.fetchMenu.drinks), 'drinks');
  });
  return price;
};
