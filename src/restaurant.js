...
function valueFoods(object) {
  let total = 0;
  const foods = Object.keys(object.fetchMenu.food);
  for (let index = 0; index < object.consumption.length; index += 1) {
    for (let item = 0; item < foods.length; item += 1) {
      if (object.consumption[index] === foods[item]) {
        total += object.fetchMenu.food[foods[item]];
      }
    }
  }
  return total;
...
function valuePay(object) {
  let total = valueFoods(object);
  const drinks = Object.keys(object.fetchMenu.drink);
  for (let index = 0; index < object.consumption.length; index += 1) {
    for (let item = 0; item < drinks.length; item += 1) {
      if (object.consumption[index] === drinks[item]) {
        total += object.fetchMenu.drink[drinks[item]];
      }
    }
  }
  return total;
